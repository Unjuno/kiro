...
const ROOT_CHOICE_LABELS = {
  Helen: {
    "H-1": "Defy her family and marry Jed.",
    "H-2": "Refuse the marriage and follow her family's advice."
  },
  Jed: {
    "J-1": "Marry Gwen.",
    "J-2": "Do not marry Gwen."
  },
  Saunders: {
    "S-1": "Lend the money to Alfred for college.",
    "S-2": "Invest the money in the store."
  }
};
...
function stripHeadings(paragraphs, node) {
  return paragraphs.filter((p, index) => {
    if (
      index < 5 &&
      p.includes("Consider the Consequences!") &&
...
export async function loadConsiderScene(node) {
  ...
  const choices = targets.map((target) => ({
    target,
    label: ROOT_CHOICE_LABELS[node]?.[target] || choiceLabel(choiceParagraph, target)
  }));
