...
const TEXT = {
  en: {
    ...
    characters: 'Choose where to begin', choices: 'What do you choose?', ending: 'This path has ended',
  ...
  ja: {
    ...
    characters: '開始する視点を選ぶ', choices: 'どうしますか？', ending: 'このルートの結末です',
...
function bookCover(story,lang) {
  if(story.id!=='consider-the-consequences') {
    return `<figure class="book-figure"><div class="book-cover generic-book"><p class="book-imprint">KIRO <span>${esc(story.language.toUpperCase())}</span></p><div class="book-title" lang="${esc(story.language)}">${esc(story.title)}</div><p class="book-authors">${(story.authors??[]).map(esc).join('<br>')}</p><div class="book-rule" aria-hidden="true"></div><p class="book-year">${esc(story.year??'')}</p></div></figure>`;
  }
  const d=design(lang);
...
function protocol(lang,stage,text,state={}) {
  ...
  const sameState=href({...state,lang:stage==='language'?'':lang});
  const agentUrl=sameState.includes('?') ? `${sameState}&format=json` : '/?format=json';
  const agentLabel=/^ja(?:-|$)/i.test(lang) ? 'エージェント用の現在ページを開く' : 'Open current state for a voice agent';
  return `<aside class="agent" id="voice-guide" data-kiro-agent-protocol><p class="agent-summary">${esc(short)}</p><details><summary>${esc(text?.agent??'Voice-agent instructions')}</summary><p>${esc(instructions)}</p></details><p><a data-kiro-agent-link href="${esc(agentUrl)}">${esc(agentLabel)}</a></p></aside>`;
}
...
function attribution(story,text,lang) {
  ...
  return `<section class="meta" id="source" data-kiro-attribution><p class="eyebrow">${esc(text.source)}</p><p data-kiro-spoken-attribution lang="${esc(story.attribution_language??story.language??'en')}">${esc(story.attribution)}</p><details><summary>${esc(design(lang).sourceDetails)}</summary><p>${esc(text.credit)}</p><p lang="en">${esc(note)}</p><p><a href="${esc(story.source_url)}" rel="noreferrer">${esc(story.source_label??new URL(story.source_url).hostname)}</a>${story.rights?.wiki_contributions_license_url==='https://creativecommons.org/licenses/by-sa/4.0/'?' · <a href="https://creativecommons.org/licenses/by-sa/4.0/" rel="noreferrer">CC BY-SA 4.0 — applicable Wikisource contributions</a>':''}</p></details></section>`;
}
function failure(state,reason='invalid',status=404) {
  const text=ui(state.lang);
  return page(`<section class="error-card"><p class="eyebrow">KIRO / ${status}</p><h1>${esc(text[reason])}</h1><p>${esc(text.help)}</p>${toolbar(state,text)}${protocol(state.lang,'error',text,state)}</section>`,state.lang,'error',status);
}
function modes(lang) {
  const d=design(lang);
...
function storyDescription(story,lang) {
  if(typeof story.description==='string') return story.description;
  const base=String(lang||'').split('-')[0];
  return story.description?.[lang]??story.description?.[base]??story.description?.[story.language]??story.description?.en??'';
}
function browserNotice(story,lang) {
  if(story.language==='en') return ui(lang).browser;
  return /^ja(?:-|$)/i.test(lang)
    ? `本文の原文言語は ${story.language} です。ブラウザ単体では自動翻訳されません。音声エージェントは現在の場面と選択肢だけを選択言語に訳してください。`
    : `The canonical source language is ${story.language}. Browser text is not automatically translated. A compatible voice agent should translate only the current scene and choices.`;
}
function catalogCard(story,lang) {
  ...
  return `<article class="story-card"><div class="feature-art">${bookCover(story,lang)}</div><div class="feature-copy"><p class="eyebrow">${esc(d.format)} <span class="eyebrow-rule"></span> ${esc(story.year??'')}</p><h2>${esc(story.title)}</h2><p class="author-line" lang="en">${(story.authors??[]).map(esc).join(' &amp; ')}</p><p class="story-description">${esc(storyDescription(story,lang))}</p>${stats}${anchor(d.enter,{lang,story:story.id,v:story.version},'button-primary')}<p class="feature-footnote">${esc(d.reading)}</p></div></article>`;
}
function characterCard(entry,state,story) {
  const d=design(state.lang);
  const person=story.id==='consider-the-consequences'?cast[entry.id]:null;
  const action=person?d.choose:ui(state.lang).next;
  return `<a class="character-card" href="${esc(href({...state,node:entry.id}))}">${person?portrait(entry.id):''}<span class="character-copy"><span class="character-name" lang="${esc(story.language)}">${esc(entry.label)}</span>${person?`<span class="character-theme">${esc(person[/^ja(?:-|$)/i.test(state.lang)?'ja':'en'])}</span>`:''}<span class="character-action">${esc(action)} <span aria-hidden="true">↗</span></span></span></a>`;
}
...
export function renderRequest(input,library) {
  ...
  if(!state.node){
    return page(`<section class="story-intro"><div class="intro-copy"><p class="eyebrow">03 <span class="eyebrow-rule"></span> ${esc(d.original)} / ${esc(story.year??'')}</p><h1>${esc(story.title)}</h1><p class="author-line" lang="en">${(story.authors??[]).map(esc).join(' &amp; ')}</p><p class="story-description">${esc(storyDescription(story,selected))}</p>${attribution(story,text,selected)}</div>${bookCover(story,selected)}</section><section class="character-section"><div class="section-heading"><p class="eyebrow">${esc(text.characters)}</p><h2>${esc(story.id==='consider-the-consequences'?d.people:(story.entries.length===1?(selected.startsWith('ja')?'物語を始める':'Begin this story'):(selected.startsWith('ja')?`${story.entries.length}つの開始地点。どこから始めますか。`:`${story.entries.length} starting points. Where will you begin?`)))}</h2></div><div class="character-grid">${story.entries.map(entry=>characterCard(entry,state,story)).join('')}</div><details class="content-warning"><summary>${selected==='ja'?'作品の内容に関する注意':'Content note'}</summary><p lang="en">${esc(story.content_notice??'')}</p></details></section>${toolbar(state,text)}${protocol(selected,'story',text,state)}`,selected,'story');
  }
  ...
  return page(`<article class="reader${ending?' reader-ending':''}" data-kiro-state="${esc(node.id)}" data-kiro-version="${esc(story.version)}"><header class="reader-heading"><p class="eyebrow">${esc(ending?text.ending:text.scene)} <span class="eyebrow-rule"></span> ${esc(node.id)}</p>${ending?'<img class="ending-mark" src="/art/branch-mark.svg" alt="" width="90" height="90">':''}<h1>${esc(story.title)}</h1><div class="reader-rule" aria-hidden="true">◆</div></header><div class="prose" lang="${esc(story.language)}" data-kiro-visible-story-text>${paragraphs(node.text)}</div>${choices}<aside class="reading-note"><p class="save-heading">${esc(d.resume)}</p><p class="save-note">${esc(text.download)}</p><p class="browser-note">${esc(browserNotice(story,selected))}</p></aside>${attribution(story,text,selected)}<p class="source-link"><a href="${esc(node.source.url)}" rel="noreferrer">Source: ${esc(node.id)}</a></p>${toolbar(state,text)}${protocol(selected,'scene',text,state)}</article>`,selected,ending?'ending':'scene');
}
